...
// The shared progress dashboard body.
//
// A teacher (`/teacher`, ?scope=class) and a parent (`/parent`, ?scope=parent)
// look at exactly the same thing: per-student, per-subject progress — lessons
// marked complete, interactive labs opened and completed, Extra Practice / Mock
// Exam / Knowledge Check attempts with best and latest scores, and when they
// were last active. The owner asked for the parent dashboard to show "the same
// info as teacher dashboard", so the renderer lives HERE once and both pages
// wrap it in their own access rules and copy.
//
// Everything that is genuinely page-specific is a prop: the words for a person
// whose progress this is ("student" / "child"), the empty state, the footnote,
// and an optional `header` slot (the teacher's link card lives there).
//
// This component renders what the server already decided. It never fetches and
// never asks for a wider roster — the pages do the one request, and the server
// fails closed.
...
export default function StudentProgressDashboard({
  ...
  const people = `${personNoun}${roster.length === 1 ? "" : "s"}`;

  // Lab rows arrive as ids (subject_id + lesson_id) because that is what the
  // lesson page records. Resolve them to the Subject -> Module -> Lesson names a
  // teacher or parent recognises. Best-effort: when the content fetch fails the
  // rows fall back to the raw lesson id instead of showing nothing.
  useEffect(() => {
    const ids = new Set();
  ...
  return (
    <>
      <div className="tp-summary">
        <span><strong>{roster.length}</strong> linked {people}</span>
      ...
      <div className="tp-controls">
        <input
          className="tp-filter"
          type="search"
          placeholder={`Filter by ${personNoun} email`}
          value={filter}
      ...
      {filtered.length === 0 ? (
        <div className="tp-notice">
          <h2>{roster.length === 0 ? emptyTitle : "Nobody matches that filter"}</h2>
          <p className="tp-muted">
            {roster.length === 0 ? emptyCopy : "Clear the filter to see everyone."}
          </p>
        </div>
      ) : (
              ...
              <li key={s.userId} className="tp-student">
                ...
                {open && (
                  <div className="tp-student-body">
                    {s.subjects.length === 0 ? (
                      <p className="tp-muted">No activity recorded for this {personNoun} yet.</p>
                    ) : (
      ...
      {footnote && <p className="tp-footnote">{footnote}</p>}
    </>
  );
}
